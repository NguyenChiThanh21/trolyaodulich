...
const ViewCar = () => {
  ...
  const HandleEdit = (_id) => {
    setEditing(_id);
    const token = localStorage.getItem("token");
    fetch(`http://localhost:3005/category/${_id}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
  ...
  const handldUpdate = () => {
    ...
    fetch(`http://localhost:3005/category/${editing}`, {
      method: "PUT",
      headers: {
  ...
  const handleAddData = async () => {
    const newData = { name: danhMuc, describe: moTa };
    try {
      const token = localStorage.getItem("token");
          ...
          <table className="tbl">
            ...
            {data.map((u, index) => (
              ...
              >
                ...
                <td data-lable="Tên danh mục" key={u.name}>
                  {u.name}
                </td>
